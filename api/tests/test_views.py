...
class TestResourceViewDetails(APITestCase):
    ...
    def test_get_resource_details(self):
        response = self.api_client.get(reverse('resource-detail', args=[self.resource.user_id]), format='json')
    ...
    def test_update_or_delete_a_resource_without_credentials_gives_not_authenticated(self):
        response = self.api_client.put(reverse('resource-detail', args=[self.resource.user_id]), format='json')
...
class TestResourceViewDetailsWithAuth(APITestCase):
    ...
    def test_create_a_resource_when_post_is_not_allowed_gives_405_method_not_allowed(self):
        response = self.api_client.post(reverse('resource-detail', args=[self.resource.user_id]), format='json')
    ...
    def test_update_a_resource_based_on_email_id_with_valid_content(self):
        ...
        response = self.api_client.put(reverse('resource-detail', args=[self.resource.user_id]),
                                       format='json', data=data)

        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_delete_a_resource(self):
        url = reverse('resource-detail', args=[self.resource.user_id])
